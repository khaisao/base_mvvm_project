...
//    }.flowOn(Dispatchers.IO)
    ...
    fun handleResponse(response: HomeResponse): ArrayList<HomePageItem> {
        ...
        listHomePageData.add(HomePageItem.SlideHome(getListImage()))

        listHomePageData.add(HomePageItem.TitleHome("Songs"))
//        response.listSong?.take(100)?.forEach {
//            listHomePageData.add(SongHome(it))
//        }

        listHomePageData.add(HomePageItem.TitleHome("Albums"))
        response.listAlbum?.take(100)?.forEach {
            listHomePageData.add(HomePageItem.AlbumHome(it))
        }
