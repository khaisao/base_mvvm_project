...
class HomePageAdapter(
    ...
    override fun getItemViewType(position: Int) = when (getItem(position)) {
        is HomePageItem.SlideHome -> R.layout.item_home_slide_layout
        is HomePageItem.SongHome -> R.layout.item_song_layout
        is HomePageItem.TitleHome -> R.layout.item_title_home_layout
        is HomePageItem.AlbumHome -> R.layout.item_album_layout
        else -> 0
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            is HomeSlideViewHolder -> {
                holder.bindData(getItem(position) as HomePageItem.SlideHome)
            }

            is HomeSongHolder -> {
                holder.bindData(getItem(position) as HomePageItem.SongHome)
            }

            is HomeTitleHolder -> {
                holder.bindData(getItem(position) as HomePageItem.TitleHome)
            }

            is HomeAlbumHolder -> {
                holder.bindData(getItem(position) as HomePageItem.AlbumHome)
            }
        }
    }
...
class HomeAlbumHolder(val binding: ItemAlbumLayoutBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    fun bindData(data: HomePageItem.AlbumHome) {
...
class HomeSongHolder(val binding: View) : RecyclerView.ViewHolder(binding) {
    ...
    fun bindData(data: HomePageItem.SongHome) {
...
class HomeTitleHolder(val binding: ItemTitleHomeLayoutBinding) :
    ...
    fun bindData(data: HomePageItem.TitleHome) {
...
class HomeSlideViewHolder(
    ...
    fun bindData(data: HomePageItem.SlideHome) {
...
class HomePageDiffUtil : DiffUtil.ItemCallback<HomePageItem>() {
    override fun areItemsTheSame(oldItem: HomePageItem, newItem: HomePageItem): Boolean {
        if (oldItem is HomePageItem.SlideHome && newItem is HomePageItem.SlideHome) {
...
fun compareListSlide(oldItem: HomePageItem.SlideHome, newItem: HomePageItem.SlideHome): Boolean {
